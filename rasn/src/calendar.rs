...
// https://github.com/briansmith/webpki/blob/18cda8a5e32dfc2723930018853a984bd634e667/src/calendar.rs
...
mod tests {
    ...
    fn test_time_from_ymdhms_utc() {
        ...
        // year boundary
        assert_eq!(
            UtcTime::from_seconds_since_epoch(1483228799),
            time_from_ymdhms_utc(2016, 12, 31, 23, 59, 59).unwrap()
        );
        assert_eq!(
            UtcTime::from_seconds_since_epoch(1483228800),
            time_from_ymdhms_utc(2017, 1, 1, 0, 0, 0).unwrap()
        );

        // not a leap year
        assert_eq!(
            UtcTime::from_seconds_since_epoch(1492449162),
            time_from_ymdhms_utc(2017, 4, 17, 17, 12, 42).unwrap()
        );

        // leap year, post-feb
        assert_eq!(
            UtcTime::from_seconds_since_epoch(1460913162),
